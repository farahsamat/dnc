...
class Memory:
    ...
    def update_link_matrix(self, precedence_vector, link_matrix, write_weighting):
        ...
        reset_factor = 1 - dnc.utility.pairwise_add(write_weighting, is_batch=True)
    ...
    def update_read_weightings(self, lookup_weightings, forward_weighting, backward_weighting, read_mode):
        ...
        updated_read_weightings = tf.squeeze(updated_read_weightings, [0])
    ...
    def update_read_vectors(self, memory_matrix, read_weightings):
        ...
        """
        #read_weightings = tf.squeeze(read_weightings, [1])
        #read_weightings = tf.reshape(read_weightings, [1, 256, 4])
        #memory_matrix = tf.expand_dims(memory_matrix, 0)

        updated_read_vectors = tf.matmul(memory_matrix, read_weightings, adjoint_a= True)#, adj_x=True)
