...
diagnosis_codes = pd.read_csv('diagnosis_codes.csv', sep=',')
diagnosis_codes.sort_values("SUBJECT_ID", inplace=True)

# Discharge notes table
discharge_notes_all = pd.read_csv('discharge_notes.csv', sep=',')
...
x = dataset.TEXT
with open('x_raw.pickle', 'wb') as f:
        pickle.dump(x, f)

print("Pickling raw output data...")
with open('y_raw.pickle', 'wb') as f:
        pickle.dump(y50, f)
